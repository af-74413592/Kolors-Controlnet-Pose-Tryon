...
def infer( garment_image_path='./00000_00.jpg', ip_image_path = './00001_00.jpg',  prompt='图片上的模特穿着白色T恤上面有着红色Levis字块，搭配银色的耳环。', model_type = 'Pose' ):
    ...
    garment = Image.open(garment_image_path)
    ...
    im1 = np.array(condi_img)
    im2 = np.array(garment)

    condi_img = Image.fromarray( np.concatenate( (im1, im2), axis=1 ) )
    print(condi_img.size)
